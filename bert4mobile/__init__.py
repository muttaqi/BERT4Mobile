...
def trace_mobile(model, text):

    input_ids,input_mask,segment_ids,valid_ids = Ner.preprocess(model, text)

    input_ids = torch.tensor([input_ids],dtype=torch.long,device="cpu")
    input_mask = torch.tensor([input_mask],dtype=torch.long,device="cpu")
    segment_ids = torch.tensor([segment_ids],dtype=torch.long,device="cpu")
    valid_ids = torch.tensor([valid_ids],dtype=torch.long,device="cpu")

    script_model = trace(model.model, input_ids, segment_ids, input_mask,valid_ids)
...
class TracedModule(ScriptModule):
    ...
    def __init__(self, orig, id_set=None, _compilation_unit=None):
        ...
        script_module = create_script_module_for_tracing(tmp_module, ())
